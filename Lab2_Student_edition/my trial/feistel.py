...
class feistel:
    ...
    def round(self, L_block:str, R_block:str, subkey:int) -> list:
        ...
        returns [L_next , R_next] which is a list consisting of two binary blocks
        """
        ...
        R_next = int(h,2) ^ int(L_block,2)
        R_next = bin(R_next)[2:].zfill(len(L_block))
    ...
    def keyScheduler(self)->list:
        ...
        key_binary_string = bin(self.key)[2:]
        ...
        subkey_list= []
        subkey_size = len(key_binary_string) / self.num_rounds
        while(subkey_size - int(subkey_size) != 0.0):
            key_binary_string = '0' + key_binary_string
            subkey_size = len(key_binary_string) / self.num_rounds

        subkey_size = int(subkey_size)
        for r in range(self.num_rounds):
            subkey = key_binary_string[r*subkey_size: (r+1)* subkey_size]
    ...
    def feistel_network(self, message, encrypt_flag:bool):
        ...
        if(type(message) == type(1)):
            message_binary = bin(message)[2:]
        elif(not re.match("^[01]*", message)):
            message_binary = ''.join([bin(ord(i))[2:] for i in message])
        else:
            message_binary = message
        
        # TODO pad the message binary string with MSB zeros to make sure it is splitable by half
        mid = len(message_binary) / 2
        if(mid % 2 != 0):
        ...
        L_block, R_block = message_binary[:mid], message_binary[mid:]
        
        
        # TODO generate list of subkeys using keyScheduler() function
        subkeys_list = self.keyScheduler()
        ...
        for round_idx in range(self.num_rounds):
            L_block, R_block = self.round(L_block=L_block, R_block=R_block, subkey=int(subkeys_list[round_idx], 2))
